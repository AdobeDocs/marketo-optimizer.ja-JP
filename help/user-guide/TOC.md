---
user-guide-title: Marketo Optimizerのドキュメント
user-guide-description: Adobe Marketo Optimizerの詳細と、AIを活用してパーソナライズされたコンテンツをB2B リードやアカウントに提供し、マーケティング部門とセールス部門の連携を強化する方法について解説します。
source-git-commit: 7053281563adbce7b5eb6fd1669974bad15677cb
workflow-type: tm+mt
source-wordcount: '254'
ht-degree: 44%
---

# Marketo Optimizer ユーザーガイド {#user}

+ [Adobe Marketo Optimizerのドキュメント](guide-overview.md)
+ [データアーキテクチャ](data-architecture.md)
+ 基本を学ぶ {#start}
  + [チェックリストを設定](./start/setup-check-list.md)
  + [ユーザー管理](./start/user-management.md)
  + [メールの配信品質](./start/email-deliverability.md)
+ [ホームページ](./home-insights/home-page.md)
+ 共同作業者の能力 {#coworker}
  + [チャットインターフェイス](./agents/chat-interface.md)
  + [スキル](./agents/skills.md)
  + [概要からプログラムを作成](./agents/program-from-brief.md)
  + [プログラム用オーディエンスの作成](./agents/audience-creation.md)
  + [ペルソナ別にメールコンテンツをパーソナライズ](./agents/personalize-content.md)
  + [カスタムスコアリングモデルの作成](./agents/lead-scoring-model.md)
  + [ウェビナーの作成とプロモーション](./agents/webinar-creation.md)
  + [ジャーニーの進行状況の監視とデバッグ](./agents/journey-observability.md)
  + [インテントの設定と分析](./agents/intent.md)
  + [分析レポートの生成](./agents/surface-analytics.md)
+ AIを活用した意思決定 {#ai-decisioning}
  + [概要](./agents/ai-decisioning.md)
  + [AIを活用した意思決定](./agents/ai-decisioning-health.md)
+ マーケティング管理 {#marketing-management}
  + [概要](./marketing/marketing-management.md)
  + プログラム {#programs}
    + [概要](./marketing/programs.md)
    + [パーソナライゼーション用のカスタムトークン](./marketing/personalization-my-tokens.md)
  + [顧客ジャーニー](./marketing/person-journeys.md)
  + ジャーニーキャンバスとノード {#journey-nodes}
    + [ノードの概要](./marketing/person-journey-nodes.md)
    + [顧客オーディエンス](./marketing/person-audience-node.md)
    + [アクションの実行](./marketing/action-nodes.md)
    + [イベントをリッスン](./marketing/listen-for-event-nodes.md)
    + [待機](./marketing/wait-nodes.md)
    + [パスの分割と結合](./marketing/split-merge-paths-nodes.md)
    + [次に最適なパス](./marketing/next-best-path.md)
  + メールチャネル {#email-channel}
    + [概要](./marketing/email-channel.md)
    + [メール送信時間の最適化](./marketing/email-send-time-optimization.md)
    + [コンテンツのオーサリング](./content/email-authoring.md)
    + [ダークモードのデザイン](./content/email-dark-mode.md)
  + [WhatsApp チャネル](./content/whatsapp-authoring.md)
  + ウェビナーチャネル {#webinar-channel}
    + [概要](./marketing/webinars-overview.md)
    + [ウェビナーの作成](./marketing/create-webinar.md)
    + [ウェビナージャーニー](./marketing/webinar-journeys.md)
+ [ジャーニー交通制御](./marketing/journey-traffic-control.md)
+ レポート {#reports}
  + [概要](./reports/reports-overview.md)
  + [ユーザジャーニーの概要](./reports/person-journey-overview-report.md)
  + [個人ジャーニー](./reports/person-journey-individual-report.md)
  + [エンゲージメント](./reports/engagement-report.md)
  + [メールエンゲージメント](./reports/email-engagement-report.md)
  + [web エンゲージメント](./reports/web-engagement-report.md)
  + [Experience Platform データセット](./reports/aep-datasets.md)
+ オーディエンス {#audiences}
  + [ユーザーリスト](./audiences/people-lists.md)
  + [顧客の詳細](./audiences/person-details.md)
  + [イベントベースのオーディエンス](./audiences/event-based-audiences.md)
  + [ユーザーエンゲージメントスコア](./audiences/engagement-scores.md)
  + [インテントモデル設定](./audiences/intent-configuration.md)
  + [派生ペルソナ](./audiences/personas.md)
  + [宛先](./audiences/destinations.md)
+ コンテンツ {#content}
  + アセット {#assets}
    + [概要](./content/digital-asset-management.md)
    + [C2PA メタデータ](./content/c2pa-metadata.md)
  + テンプレート {#templates}
    + [概要](./content/templates.md)
    + [テンプレートの作成](./content/templates-create.md)
    + [コンテンツガバナンス](./content/template-content-governance.md)
  + フラグメント {#visual-fragments}
    + [概要](./content/fragments.md)
    + [フラグメントオーサリング](./content/fragment-authoring.md)
  + ランディングページ {#landing-pages}
    + [概要](./content/landing-pages.md)
    + [ランディングページの作成と公開](./content/landing-pages-create-publish.md)
    + [ランディングページのデザイン](./content/landing-page-design.md)
  + フォーム {#forms}
    + [概要](./content/forms.md)
    + [フォームのデザイン](./content/form-design.md)
  + [パーソナライズ機能](./content/personalization-expressions.md)
  + [条件付きコンテンツ](./content/conditional-content.md)
  + コンテンツデザインツール {#content-design}
    + [構造コンポーネント](./content/structure-components.md)
    + [コンテンツコンポーネント](./content/content-components.md)
    + [カスタム CSS](./content/design-custom-css.md)
+ [スコアリングスタジオ](./labs/scoring-studio.md)
+ [プライバシーの管理](./admin/privacy-management.md)
+ 管理 {#admin}
  + チャネル {#channels}
    + [メールチャネル設定](./admin/email-channel-configuration.md)
    + [WhatsApp チャネル設定](./admin/configuration-channels-whatsapp.md)
    + [ランディングページの設定](./admin/configuration-presets-landing-pages.md)
    + [フォームプリセット設定](./admin/configuration-presets-forms.md)
  + [ビジネスルール](./admin/business-rules.md)
  + [プログラムタイプ](./admin/program-types.md)
  + [ペルソナマッピング](./admin/persona-mapping.md)
