...
>title="ルールタイプ"
>abstract="チャネルルールセットに必要なルールタイプを選択します。**フリークエンシーキャップ**&#x200B;タイプを使用して、通信チャネルにキャッピングルールを適用します。 例えば、1 日に 1 つを超えるメールや SMS 通信を送信しないようにします。 **メッセージが通知されない時間帯**&#x200B;を選択して、時間ベースの除外を定義し、特定の期間にメッセージが送信されないようにします。"

>[!CONTEXTUALHELP]
>id="ajo-b2b-prime_rule_sets_duration"
>title="キャップ頻度のリセット"
>abstract="キャップカウンターのリセットに使用するカレンダー期間（時間単位、日単位、週単位、月単位）を選択します。 カウンターは、新しい期間の開始時に自動的に 0 にリセットされます。"
...
チャネルやコミュニケーションの種類ごとに配信頻度の上限を設定することで、プロファイルが受け取るメッセージ数を制限し、同様のコミュニケーションを顧客に過剰に配信するのを防ぎます。 チャネルルールセットは、通信チャネルにキャッピングルールを適用します。 例えば、1 日に 1 つを超えるメールや SMS 通信を送信しないようにします。
